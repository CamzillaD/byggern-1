...
#ifndef CAN_READ_QUEUE_SIZE
#define CAN_READ_QUEUE_SIZE 16
#endif

static CanFrame m_can_queue[CAN_READ_QUEUE_SIZE];
static uint8_t m_can_head = 0;
static uint8_t m_can_tail = 0;

ISR(INT2_vect){
    CanFrame * p_frame = m_can_queue + m_can_tail;
    ...
    network_write_can_message(p_frame, 0);

    m_can_tail = (m_can_tail + 1) % CAN_READ_QUEUE_SIZE;
}
...
void can_init(){
    ...
    /* FIFO1: Receive FIFO, no timestamp, interrupt on receive */
    mcp2518fd_sfr_write(MCP_SFR_C1FIFOCON(1, 0), 0x01);
...
uint8_t can_write(const CanFrame * p_frame){
    ...
    network_write_can_message(p_frame, 1);
...
uint8_t can_read(CanFrame * p_frame){
    ...
    p_frame->id = m_can_queue[m_can_head].id;
    p_frame->size = m_can_queue[m_can_head].size;

    for(uint8_t i = 0; i < p_frame->size; i++){
        p_frame->buffer[i] = m_can_queue[m_can_head].buffer[i];
    }

    m_can_head = (m_can_head + 1) % CAN_READ_QUEUE_SIZE;
